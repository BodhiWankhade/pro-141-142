...
def recommended_movies():
    global liked_movies
    colm_names=["orginal_title","poster_link","release_date","runtime","weighted_rating"]
    all_recommended=[]
    for like_movie in liked_movies:
        output=get_recommendations(like_movie["original_title"])
        all_recommended=all_recommended.append(output)
    #all_recommended.drop_duplicates(subset=["original_title"],inplace=True)
    recommended_movies_data=[]
    for intex,row in all_recommended:
        _p = {
            "original_title": row['original_title'],
            "poster_link":row['poster_link'],
            "release_date":row['release_date'] or "N/A",
